...
import { KEY_CODES } from '@vinicunca/js-utilities';

import VFocusTrap from '../src/focus-trap.vue';
import { INJECTION_KEY_FOCUS_TRAP } from '../src/constants';
...
describe('VFocusTrap', () => {
  ...
  function createComponent(props = {}, items = 0) {
    return mount(VFocusTrap, {
      props: {
        trapped: true,
        ...props,
      },
  ...
  describe('events', () => {
    ...
    it('should be able to dispatch `releaseRequested` if escape key pressed while trapped', async () => {
      wrapper = createComponent(
        { trapped: false, loop: true },
        1,
      );
      await nextTick();
    ...
    it('should be able to dispatch `focusoutPrevented` when trab wraps due to trapped or is blocked', async () => {
      wrapper = createComponent(undefined, 3);
      await nextTick();
      await nextTick();
