...
describe('workspace', () => {
  ...
  it('getDependencies should work', async () => {
    const pkgs = await getWorkspacePackages();
    const pkg = pkgs['@vinicunca/build-utils'];
  ...
  describe('getPackage should work', () => {
    it('with package name and prefix', async () => {
      const pkg = await getPackage('@vinicunca/build-utils');
      expect(pkg?.manifest.name).toBe('@vinicunca/build-utils');
    });
    ...
    it('without package name', async () => {
      const _pkg = await getPackage('@vinicunca/build-utils');
